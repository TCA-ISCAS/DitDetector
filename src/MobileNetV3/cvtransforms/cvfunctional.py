...
def my_resize(img, size, interpolation='BILINEAR'):
    """Resize the input CV Image to the given size.

    Args:
        img (np.ndarray): Image to be resized.
        size (tuple or int): Desired output size. 
        如果是tuple，那就按这个尺寸resize，如果是int，就将长边resize到这个大小，短边根据原比例放缩
        interpolation (str, optional): Desired interpolation. Default is ``BILINEAR``
...
def pre_ratio_resize_and_pad(img, padding, fill=(0, 0, 0), padding_mode='constant'):
    """
    Args:
        img (np.ndarray): Image to be padded.
        padding (int): 
        输入int 长边等比例放缩到这个尺寸之后，再把短边等比例放缩，然后padding到这个尺寸
    ...
    if h>w:
        ratio = target_size/h
        new_w = math.floor(ratio*w)
        new_h = target_size
        pad = int((target_size-new_w)/2)
        img = cv2.resize(img,(new_w,new_h),interpolation=cv2.INTER_LINEAR)
        image_temp = cv2.copyMakeBorder(img, 0, 0, pad, target_size-pad-new_w, cv2.BORDER_CONSTANT, value=(0,0,0))

    else:
        ratio = target_size/w
        new_w = target_size
        new_h = math.floor(ratio*h)
        pad = int((target_size-new_h)/2)
        img = cv2.resize(img,(new_w,new_h),interpolation=cv2.INTER_LINEAR)
        image_temp = cv2.copyMakeBorder(img, pad, target_size-pad-new_h,0,0, cv2.BORDER_CONSTANT, value=(0,0,0))
...
def random_erasing(img,sl,sh,r1):
    img_h,img_w,img_c = img.shape
    for _ in range(20):
        img_area = img_h * img_w
        mask_area = np.random.uniform(sl, sh) * img_area
        mask_aspect_ratio = np.random.uniform(r1, 1/r1)
        ...
        if right >= img_w or bottom >= img_h:
                continue
        # img[top:bottom, left:right] = mask[:min(bottom-top,img_h-top),:min(right-left,img_w-left)]
