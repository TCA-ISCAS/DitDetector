...
__all__ = ["Compose", "ToTensor", "ToCVImage",
           "Normalize", "Resize", "PreRatioResizeAndPad","CenterCrop", "Pad",
           "Lambda", "RandomApply", "RandomOrder", "RandomChoice", "RandomChooseTwo",
           "RandomCrop",
...
class RandomErasing(object):
    ...
    def __init__(self, p=0.5, sl=0.02, sh=0.1, r1=0.3, mean=(0.4914, 0.4822, 0.4465)):
        self.p = p
        self.sl = sl
        self.sh = sh
        self.r1 = r1
    ...
    def __call__(self, img):
        ...
        if random.random() < self.p:
            return F.random_erasing(img, self.sl,self.sh,self.r1)
        return img

    def __repr__(self):
        return self.__class__.__name__ + '(p={0},sl={1},sh={2}),r1={3}'.format(self.p,self.sl,self.sh,self.r1 )
...
class PreRatioResizeAndPad(object):
    """
    ...
    def __call__(self, img):
        ...
        Returns:
            CV Image: Padded image.
        """
        return F.pre_ratio_resize_and_pad(img, self.padding, self.fill, self.padding_mode)
...
class RandomChooseTwo(RandomTransforms):
    """Apply two transformation randomly picked from a list
    """
    def __call__(self, img):
        two = random.sample(self.transforms,2)
        for i in two:
            img = i(img)
        return img
