...
/**
 * The <code>Export</code> class represents the interface for performing file conversions.  These conversions are
 * done by making calls to the {@link #convert(String, String, String) convert} method, which can be called repeatedly
 * for any instance of this class. It also allows for the setting of conversion properties that are persistant across
 * <code>convert</code> calls.
 *
 * @author John Pazdernik
 * @version 1.00
 * @see <a href=http://www.java.sun.com/j2se/1.3/docs/api/java/util/Properties.html>java.util.Properties</a>
 */
public class Export {
    ...
    */
    ...
    /**
    * Constant strings containing some additional Formats not exposed in <code>ExportFormat</code>.
    */
    ...
    */
    ...
    /**
     * String representing the default name of the process to execute in <code>convert</code>.  This name can be
     * overridden by specifying a full pathname in the EXEPATH property.
     */
     ...
     */
    public Export() {
        // Determine the name of the directory which contains the Class resource for this object.
        // This directory will be used as the default expected location for the commandName exe.
    ...
    /**
     * Creates a new <code>Export</code> object with the supplied properties.  These properties are copied to protect
     * them from modifications external to this object.
     *
     * @param properties The properties to use when executing conversions.
     */
     ...
     *
     * @return A copy of the properties currently associated with this conversion.
     */
    ...
    /**
     * Sets the properties to be used for all subsequent conversions executed by this <code>Export</code> object.
     * These properties are copied to protect them from modifications external to this object.
     *
     * @param properties The properties to use when executing conversions.
     */
    ...
    /**
     * Convert a file by executing a native process with no timeout.
     * Use the supplied parameters and the previously set properties
     * to construct a command line String array.  Execute this command line using a Runtime.exec method, wait for
     * the process to complete, capture the return value, wrap it in an {@link ExportStatusCode ExportStatusCode}
     * and return.
     *
     * @param inputPath  The full pathname of the file to convert.
     * @param outputPath The full pathname of the destination file for the conversion.
     * @param outputFormat
     *                   The type of format to convert the inputPath file into.  See {@link ExportFormat ExportFormat}
     *                                   for the available format types.
     * @return Return value of the conversion wrapped as an <code>ExportStatusCode</code>.
     */
    ...
    /**
     * Convert a file by executing a native process. Use the supplied parameters and the previously set properties
     * to construct a command line String array.  Execute this command line using a Runtime.exec method, wait for
     * the process to complete, capture the return value, wrap it in an {@link ExportStatusCode ExportStatusCode}
     * and return.
     *
     ...
     *                   The type of format to convert the inputPath file into.  See {@link ExportFormat ExportFormat}
     *                                   for the available format types.
     * @param timeout    Timeout interval in milliseconds; 0 means no timeout.
     * @return Return value of the conversion wrapped as an <code>ExportStatusCode</code>.
     */
    ...
    public ExportStatusCode convert( String inputPath, String outputPath, String outputFormat, long timeout ) {
        ...
        {
            if ( unix == true )
                execParams[0] = COMMAND_NAME_UNIX;
            else
                execParams[0] = COMMAND_NAME;
        }

				/* The 1.5 JVM has a bug which causes a problem with quoted UNC pathnames.
				 * I have tested unquoted names on both 1.4 and 1.5 VMs, and cannot find a
				 * definitive need for the quotes in either case, so I am simply removing them
				 */

        /* if (unix == false)
          execParams[0] = "\"" + execParams[0] + "\""; */
        ...
        // Change the output format to the value expected by the native process if the format is
        // html and the flavor is WCA or AG.
        if ( OUTPUTFORMAT == ExportFormat.HTML )
        {
            String flavor = properties.getProperty( ExportProperties.FLAVOR );
            if ( flavor != null )
            {
                if ( flavor == ExportProperties.FLAVOR_WCA11 )
                    execParams[ 3 ] = OUTPUTFORMAT + "=" + FORMAT_HTMLWCA;
                else if ( flavor == ExportProperties.FLAVOR_AG33CE || flavor == ExportProperties.FLAVOR_AG33PALM )
                    execParams[ 3 ] = OUTPUTFORMAT + "=" + FORMAT_HTMLAG;
                if (unix == false)
                  execParams[ 3 ] = "\"" +  OUTPUTFORMAT + "=" + outputFormat + "\"";
            }
        }
    ...
    private int getExitValue(Process process)  {
    	...
    	if (process != null) {
            ...
            /*
             * POSIX systems can only return an exit code between 0 and 255 from a process, so returning some
             * of the larger error codes from OIT is impossible this way.  In order to deal with this situation,
             * the native app (exporter) will now return 1 if there an an error, and 0 if successful.  In the
             * case of an error, the actual error code will be written to stderr so that it can be read by
             * the calling process.
             */
            if (exitCode != 0) {
    ...
    /**
     * Take the individual properties out of the Properties object, and if they are not the EXEPATH property, or
     * their values are not equal to DEFAULT, place them into the String array execParams.
     */
    private void parseProperties() {
        ...
        // allocate space for the number of parameters that was just determined, plus 4 slots for the
        // commandName and the 3 required parameters that will be passed to convert().
        execParams = new String[ count + 4 ];
        count = 4;
        propertyNames = properties.propertyNames();

        // Now place the properties into their params slots, encoding those that require it.
        while ( propertyNames.hasMoreElements() )
            {
            String key = ( String )propertyNames.nextElement();
            String value = properties.getProperty( key );
    ...
    /**
     * Unpacks 3 bytes into 4 locations in the destination array, and also encodes them using Base64.
     *
     * @param val    The integer containing the 3 packed bytes to be encoded.
     * @param dest   The char array where the encoded bytes are to be placed.
     * @param index  The location in the destination array to place the encoded bytes.
     */
